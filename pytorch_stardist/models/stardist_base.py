...
class StarDistBase(nn.Module):
    ...
    def set_optimizers(self):
        opt = self.opt
        self.optimizer = torch.optim.AdamW(self.net.parameters() , lr=opt.lr, betas=(opt.beta1, opt.beta2))
    ...
    def _check_for_nan(self, tensors_dict):
        nan_found = False
    ...
    def evaluate(self, batch, epoch=None):
        ...
        prob = batch['prob'].to(self.device)

        prob_class = None
        if "prob_class" in batch:
            prob_class = batch["prob_class"].to(self.device)
        ...
        with torch.no_grad():
            ...
            loss_prob_class = torch.tensor(0.).to(self.device)
        ...
        # Check for NaN values in predictions and losses
        nan_check_dict = {
            "pred_dist": pred_dist,
            "pred_prob": pred_prob,
            "Val_loss": loss,
            "Val_loss_prob": loss_prob,
            "Val_loss_dist": loss_dist,
            "Val_loss_prob_class": loss_prob_class
        }
        if pred_prob_class is not None:
            nan_check_dict["pred_prob_class"] = pred_prob_class
        
        nan_detected = self._check_for_nan(nan_check_dict)
        ...
        torch.distributed.all_reduce(loss_prob, op=torch.distributed.ReduceOp.AVG)
        torch.distributed.all_reduce(loss_dist, op=torch.distributed.ReduceOp.AVG)
        torch.distributed.all_reduce(loss_prob_class, op=torch.distributed.ReduceOp.AVG)
    ...
    def optimize_parameters(self, batch, epoch=None):
        ...
        # Check for NaN values in predictions and losses before backward pass
        nan_check_dict = {
            "pred_dist": pred_dist,
            "pred_prob": pred_prob,
            "loss": loss,
            "loss_prob": loss_prob,
            "loss_dist": loss_dist,
            "loss_prob_class": loss_prob_class
        }
        if pred_prob_class is not None:
            nan_check_dict["pred_prob_class"] = pred_prob_class
    ...
    def predict(self, image, patch_size=None, context=None):
        ...
        if patch_size is not None and context is not None:
            ...
            if isinstance(self.net, torch.nn.parallel.DistributedDataParallel):
                pred_dist, pred_prob, pred_prob_class = self.net.module.predict(image)
            else:
    ...
    def update_lr(self, epoch=None, metric=None, metric_name=""):
        ...
        lr = update_lr(self.optimizer, self.lr_scheduler, self.opt, metric=metric, do_print=(self.global_rank == 0))
    ...
    def save_state(self, name=None):
        ...
        if isinstance(self.net, torch.nn.parallel.DistributedDataParallel):
            net_state_dict = OrderedDict((k, v.cpu())for k, v in self.net.module.state_dict().items())
        else:
            net_state_dict = OrderedDict((k, v.cpu())for k, v in self.net.state_dict().items())
    ...
    def load_state(self, name=None, load_path=None):
        ...
        print("Instanciating network if not already done")
        if self.net is None:
            self.net = define_stardist_net( loaded_opt )
            print( self.net.load_state_dict( state['model_state_dict'] ) )
        else:
            if isinstance(self.net, torch.nn.parallel.DistributedDataParallel):
                print( self.net.module.load_state_dict( state['model_state_dict'] ) )
            else:
                print( self.net.load_state_dict( state['model_state_dict'] ) )
    ...
    def optimize_thresholds(
        ...
        #That_val = [self.predict(x, **_predict_kwargs(x))[:2] for x in X_val]
        ...
        for _opt_nms_thresh in nms_threshs:
            print(f'Evaluating nms threshold {_opt_nms_thresh}')
