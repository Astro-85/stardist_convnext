...
def get_params(opt, size):
    ...
    flip_axis = [-3, -2, -1, (-3, -2), (-3, -1), (-2, -1), (-3, -2, -1)][np.random.randint(3)]
    
    # Rotation params
    # ===============
    rotate = random.random() > 0.5
    rotate_amount = np.random.randint(1, 4)

    zrotate = random.random() > 0.5
    zrotate_y_amount = np.random.randint(1, 4)
    zrotate_x_amount = np.random.randint(1, 4)
    ...
    noise_amount = np.random.uniform(0, 2)
    ...
    return {'crop_pos': (z, x, y), 'flip': flip, 'flip_axis': flip_axis, 
            ...
            'rotate': rotate, 'rotate_amount': rotate_amount, 'noise': noise, 'noise_amount': noise_amount,
            
            'zrotate': zrotate, 'zrotate_y_amount': zrotate_y_amount, 'zrotate_x_amount': zrotate_x_amount,
...
def get_transforms(opt, params, is_mask=False):
    ...
    if "rotate" in opt.preprocess:
        transform_list.append( transforms.Lambda( lambda vol: __rotate(vol, params["rotate"], rotate_amount=params["rotate_amount"]) ) )
        
    if "noise" in opt.preprocess:
        if not is_mask:
            transform_list.append( transforms.Lambda( lambda vol: __noise(vol, params["noise"], noise_amount=params["noise_amount"]) ) )
    ...
    if "zrotate" in opt.preprocess:
        transform_list.append( transforms.Lambda( lambda vol: __zrotate(vol, params["zrotate"], zrotate_y_amount=params["zrotate_y_amount"], zrotate_x_amount=params["zrotate_x_amount"]) ) )
...
def __rotate(volume, rotate:bool, rotate_amount): # Rotate the x-y axis
    if rotate:
        return np.rot90(volume, k=rotate_amount, axes=(-1, -2))
    return volume

def __zrotate(volume, zrotate:bool, zrotate_y_amount, zrotate_x_amount): # Rotate the x-z and y-z axis
    if zrotate:
        volume = np.rot90(volume, k=zrotate_y_amount, axes=(-3, -2))
        volume = np.rot90(volume, k=zrotate_x_amount, axes=(-3, -1))
        return volume
    return volume

def __noise(volume, noise:bool, noise_amount): # Add gaussian noise of different strengths
    if noise:
        return volume + np.random.normal(0, noise_amount*volume.std(), volume.shape)
    return volume
