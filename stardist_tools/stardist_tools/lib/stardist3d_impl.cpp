...
#pragma omp parallel for schedule(dynamic)
    for (unsigned long long z = std::max(0,bbox[0]); z <= std::min(nz-1,bbox[1]); ++z) {
      for (unsigned long long y = std::max(0,bbox[2]); y <= std::min(ny-1,bbox[3]); ++y) {
        for (unsigned long long x = std::max(0,bbox[4]); x <= std::min(nx-1,bbox[5]); ++x) {

          bool inside = false;
          unsigned long long offset = x+y*nx+z*(nx*ny);
